...
void OdomImu::imuCB(const sensor_msgs::Imu::ConstPtr& msg)
{
  ...
  double linear_vel_x = round(msg->linear_acceleration.x / param_linear_vel_sensitive_) * param_linear_vel_sensitive_;
  double linear_vel_y = round(msg->linear_acceleration.y / param_linear_vel_sensitive_) * param_linear_vel_sensitive_;
  double linear_vel_z = round(msg->linear_acceleration.z / param_linear_vel_sensitive_) * param_linear_vel_sensitive_;
  linear_vel_y = 0.;
  linear_vel_z = 0.;
  ...
  double accX1 = linear_vel_x;
  double accY1 = std::cos(current_pose_.roll) * linear_vel_y - std::sin(current_pose_.roll) * linear_vel_z;
  double accZ1 = std::sin(current_pose_.roll) * linear_vel_y + std::sin(current_pose_.roll) * linear_vel_z;
  ...
  try
  {
    tf_listener_.waitForTransform(param_base_frame_, msg->header.frame_id, ros::Time(0), ros::Duration(0.015));
    tf_listener_.lookupTransform(param_base_frame_, msg->header.frame_id, ros::Time(0), transform1);
  }
  ...
  tf::Transform transform = transform2 * transform1.inverse();
