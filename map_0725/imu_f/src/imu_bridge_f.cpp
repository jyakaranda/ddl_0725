...
ImuBridge::ImuBridge(ros::NodeHandle nh, ros::NodeHandle pnh) : //  Members default values
                                                                nh_(nh),
                                                                pnh_(pnh),
                                                                ...
                                                                is_calibrated_(false)
{
  ...
  raw_sub_ = nh_.subscribe("raw_imu", 5, &ImuBridge::rawCallback, this);

  imu_cal_srv_ = nh_.advertiseService("imu/calibrate_imu", &ImuBridge::calibrateCallback, this);
  ...
  if (use_accelerometer_ || use_gyroscope_)
  {
    ...
    vel_pub_ = nh_.advertise<geometry_msgs::Twist>("imu/vel_raw", 5);
  ...
  if (use_magnetometer_)
  {
    ...
    if (use_mag_msg_)
    {
      ...
      ImuBridge::fillRowMajor(magnetic_field_covar_, magnetic_field_stdev_);
    }
    else
    {
  ...
  ROS_INFO("Starting Imu Bridge.");
}

void ImuBridge::rawCallback(const ros_arduino_msgs::RawImuConstPtr &raw_msg)
{
  ...
  else
  {
    if (use_accelerometer_ || use_gyroscope_)
    {
      ...
      //  ROS_INFO("imu/data_raw Liner[%.1f,%.1f,%.1f] Angular[%.1f,%.1f,%.1f]",
      //  imu_msg->angular_velocity.x,  imu_msg->angular_velocity.y,  imu_msg->angular_velocity.z,
      //  imu_msg->linear_acceleration.x, imu_msg->linear_acceleration.y, imu_msg->linear_acceleration.z);

      geometry_msgs::TwistPtr vel_msg = boost::make_shared<geometry_msgs::Twist>();

      vel_msg->linear.x = vel_msg->linear.x + imu_msg->linear_acceleration.x * (end - begin);
      vel_msg->linear.y = vel_msg->linear.y + imu_msg->linear_acceleration.y * (end - begin);
      vel_msg->linear.z = vel_msg->linear.z + (imu_msg->linear_acceleration.z + GRAVITY) * (end - begin);
      vel_msg->angular.x = imu_msg->angular_velocity.x;
      vel_msg->angular.y = imu_msg->angular_velocity.y;
...
bool ImuBridge::calibrateCallback(std_srvs::Empty::Request &request, std_srvs::Empty::Response &response)
{
...
void ImuBridge::fillRowMajor(boost::array<double, 9> &covar, double stdev)
{
