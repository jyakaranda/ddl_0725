...
bool NDTMap::saveKeyframesAndFactor()
{
  ...
  else
  {
    ...
    gtsam::Pose3 pose_to = Pose3(Rot3::RzRyRx(roll, pitch, yaw), Point3(cur_pose_ndt_.pose.pose.position.x, cur_pose_ndt_.pose.pose.position.y, cur_pose_ndt_.pose.pose.position.z));
    ...
    initial_estimate_.insert(cloud_keyposes_3d_->points.size(), Pose3(Rot3::RzRyRx(roll, pitch, yaw), Point3(cur_pose_ndt_.pose.pose.position.x, cur_pose_ndt_.pose.pose.position.y, cur_pose_ndt_.pose.pose.position.z)));
  }
...
void NDTMap::pcCB(const sensor_msgs::PointCloud2ConstPtr &msg)
{
  ...
  if (use_odom_ && odom_ptr_last_ != -1)
  {
    ...
    Eigen::Quaternionf tmp_q(cur_pose_m_.block<3, 3>(0, 0));
    double roll, pitch, yaw;
