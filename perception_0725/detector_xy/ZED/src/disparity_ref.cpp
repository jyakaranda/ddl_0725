		...
		if (zed.grab(runtime_parameters) == sl::SUCCESS) {
            ...
            for(int i=0;i<len3;i++)
			{
			  ...
			  if (distance>20)
			  {
				...
				//std::string disString("Incorrect perception distance to object  ");
				sprintf(str, "%d", i);
			  ...
			  else
			  {
				...
				// 深度
			    sprintf(disstr, "%.2f ->  %.2f ", distance,angle);
			 ...
			 /*
			  std::cout << "x:" << boxes_point[i][1] <<" "<<"y:"<<boxes_point[i][0]<<std::endl;
			  computeDistance(filtered_disp_vis,pointcloud,Q,boxes_point[i][0],boxes_point[i][1],distance01);
              std::cout << "distance01:" << distance01 <<std::endl;

			  std::cout << "x:" << boxes_point[i][3] <<" "<<"y:"<<boxes_point[i][2]<<std::endl;
              computeDistance(filtered_disp_vis,pointcloud,Q,boxes_point[i][2],boxes_point[i][3],distance02);
              std::cout << "distance02:" << distance02 <<std::endl;


              std::string disString("Distance01 and distance02 to point:");
			  sprintf(disstr, "%.2f", (distance01+distance02)/2);
...
float computeDistance(Mat& filtered_disp,Mat& pointcloud,const Mat& Q,int i,int j,int** boxes_point,float& distance)
{
    ...
    //深度 
	distance = depth.at<float>(boxes_point[i][j], boxes_point[i][j+1]);
...
float computeDistance(Mat& filtered_disp,Mat& pointcloud,const Mat& Q,float& pointy,float& pointx,float& distance)
{
	...
	distance = depth.at<float>(pointx, pointy);
